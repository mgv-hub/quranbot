...
async function saveDataLocally() {
   try {
      ...
      await fs.writeFile(
         ...
         JSON.stringify(global.surahNames, null, 2),
      );
      await fs.writeFile(
         pathlra.join(dataDir, 'reciters_ar.json'),
         JSON.stringify(global.reciters, null, 2),
      );
      await fs.writeFile(
         pathlra.join(dataDir, 'radios_ar.json'),
         JSON.stringify(global.quranRadios, null, 2),
      );
      await fs.writeFile(
         pathlra.join(dataDir, 'adhkar_ar.json'),
         JSON.stringify(global.azkarData, null, 2),
      );
