...
function registerLavalinkEvents(manager, lavalinkNodeConfig, selectBest) {
    ...
    manager.nodeManager.on('disconnect', async (node, reason) => {
        ...
        if (playersToMigrate.length > 0) {
            const connectedNodes = Array.from(manager.nodeManager.nodes.values()).filter((n) => n.connected && n.id !== node.id);
            if (connectedNodes.length === 0) {
                logger.warn(`All Lavalink nodes disconnected. Skipping failover for ${playersToMigrate.length} players from ${node.id}`);
