...
const { checkCooldown, setCooldown, getCooldownResponse } = require('@state/CommandCooldown');
...
const { getApiHeaders } = require('@config/HttpConfig');

module.exports = {
    customId: 'lavalink_status',
    async execute(interaction) {
        const cdResult = checkCooldown(interaction.user.id, interaction.guildId, 'lavalink_status');
        if (!cdResult.allowed) {
            await safeError(interaction, getCooldownResponse(cdResult.remaining, cdResult.type));
            return;
        }

        setCooldown(interaction.user.id, interaction.guildId, 'lavalink_status');
                        ...
                        components: [
                            {
                                ...
                                components: [
                                    {
                                        type: 10,
                                        content: 'لم يتم تكوين أي عقدة Lavalink',
                                    },
                                ],
                            },
                        ],
