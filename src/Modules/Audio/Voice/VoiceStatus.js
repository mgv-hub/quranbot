...
async function processQueue() {
    ...
    while (queue.size > 0) {
        ...
        try {
            ...
            if (error.status === 429) {
            ...
            } else if (error.code !== 50013 && error.status !== 403 && error.status !== 500 && error.status !== 503) {
                ...
                if (errMsg.includes('ECONNREFUSED') || errMsg.includes('ETIMEDOUT') || errMsg.includes('fetch failed')) {
                    logger.warn(`Transient network error setting voice status for ${channelId}: ${errMsg}`);
                } else {
                    logger.error(`Failed to set voice ${channelId} ${errMsg}`);
                }
            }
        }
    }
...
function generateStatusText(guildState) {
    ...
    const manualVoiceStatus = statusManager.getStatus().voiceStatus;

    if (manualVoiceStatus) {
        return manualVoiceStatus;
    }
...
module.exports.clearStatusCache = clearStatusCache;
