...
const { getBestNode, nodeConfigs } = require('@bootstrap/BotSetup');
...
async function teardownConnection(guildId, guildState) {
    ...
    if (guildState.rawConnection) {
        try {
            guildState.rawConnection.destroy();
        } catch (e) {
            voiceLogger.debug(guildId, 'Failed to destroy raw connection during teardown', { error: e.message });
        }
        guildState.rawConnection = null;
    }
    ...
    if (channelIdToClear) {
        const { setVoiceChannelStatus } = require('@modules/Audio/Voice/VoiceStatus');
        const client = require('@bootstrap/BotSetup').client;
        setVoiceChannelStatus(channelIdToClear, '', client).catch(() => {});
    }
...
async function initializeConnection(guildId, guildState, targetChannel, adapterCreator) {
    ...
    //const { resetPlayer } = require('@modules/Audio/Playback/PlayerEvents');
            ...
            if (node && node.connected) {
                ...
                if (!isFull) {
                    ...
                    voiceLogger.connection(guildId, `Using preferred node ${bestNode.id}`, {
                        location: require('@bootstrap/BotSetup').nodeConfigs.get(bestNode.id)?.location,
                    });
                } else {
