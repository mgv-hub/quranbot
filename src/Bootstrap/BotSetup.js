...
const { loadData, saveSetupGuilds } = require('@data/Management/DataManager');
const { Client, GatewayIntentBits, REST } = require('discord.js');
const { LavalinkManager } = require('lavalink-client');
const { buildLavalink, parseNodeConfig, clearNodeCache } = require('@config/LavalinkConfig');
const { registerLavalinkEvents } = require('@modules/Audio/Lavalink/LavalinkEventBus');
...
client.on('raw', (data) => {
    if (client.lavalink) {
        Promise.resolve(client.lavalink.sendRawData(data)).catch(() => {});
    }
});
