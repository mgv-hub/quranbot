...
const voiceManager = require('@modules/Audio/Voice/VoiceConnection');

let RuntimeSaveTime = 0;

async function saveRuntimeStates() {
    const now = Date.now();
    if (now - RuntimeSaveTime < 120000) {
        return true;
    }
    RuntimeSaveTime = now;
    ...
    try {
        if (global.guildStates) {
            for (const [guildId, state] of global.guildStates.entries()) {
                ...
                storedState.voiceChannelId = state.channelId;
                storedState.playbackMode = state.playbackMode;
                storedState.currentReciter = state.currentReciter;
                storedState.currentSurahIndex = state.currentSurah - 1;
                storedState.isPaused = state.isPaused;
                storedState.currentRadioIndex = state.currentRadioIndex;
