...
async function loadPrayerTimesData() {
    ...
    try {
        const dataPath = pathlra.join(__dirname, '../../Resources/PrayerTimesData.json');
        ...
        // fallback to empty structure so the bot doesn't crash
        prayerTimesData = { countries: [], citiesByCountry: {}, countryTimeFormats: {} }; // even if loading fails, we set it to an empty structure to avoid repeated load attempts
        return prayerTimesData;
    }
}
