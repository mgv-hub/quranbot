...
const { cleanState } = require('@state/PersistDefaults');
...
async function restoreGuildState(guildId, guildStates, client) {
    ...
    try {
        ...
        if (!channel) {
            return { success: false, reason: 'Voice Channel Not Found' };
        }

        // Verify bot permissions before reconnecting
        const permissions = channel.permissionsFor(guild.members.me);
        if (!permissions.has('Connect') || !permissions.has('Speak')) {
            return { success: false, reason: 'Missing Permissions' };
        }

        return { success: true, state: state, channel: channel };
    } catch (error) {
