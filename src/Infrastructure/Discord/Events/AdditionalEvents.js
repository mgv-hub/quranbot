...
const { removeGuildState, applyCommandPermissions } = require('@core/ServiceRegistry');
const { ChannelType } = require('discord.js');
const botClient = require('@bootstrap/BotSetup').client;
const http = require('http');
// const { markGuildAsLeft, markGuildAsPresent } = require('@infrastructure/Persistence/Firebase/Services/RetentionService');
const retentiondb = require('@infrastructure/Persistence/Firebase/Retention/RetentionIndex');
const { sendWelcome } = require('@infrastructure/Discord/Events/GuildWelcome');
...
botClient.on('guildCreate', async (guild) => {
    ...
    auditLogger.logGuildJoin(guild, owner);
    ...
    try {
        ...
        if (!global.setupGuilds[guild.id]) {
            const { loadSetupGuildsFromFirebase } = require('@infrastructure/Persistence/Firebase/FirebaseIndex');
...
botClient.on('guildDelete', async (guild) => {
    ...
    auditLogger.logGuildLeave(guild);
