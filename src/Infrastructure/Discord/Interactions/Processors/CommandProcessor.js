...
const { getErrorType } = require('@infrastructure/Discord/Interactions/InteractionErrors');

// Categorize commands by access level and execution context
const public_commands = ['مصادر', 'مواقيت_الصلاة', 'سرعة', 'دليل', 'تحديثات', 'مساعدة', 'تفسير', 'بحث', 'سورة', 'المسباح', 'المساهمين'];
const admin_commands = ['خروج', 'دخول', 'دخول_قناة', 'إعداد', 'تعيين_القنوات', 'تذكير_الصلاة'];
const control_commands = ['تحكم'];
const guild_required_commands = ['تحكم', 'خروج', 'دخول', 'دخول_قناة', 'إعداد', 'تذكير_الصلاة'];
...
async function executeCommand(interaction, commandName) {
    switch (commandName) {
        ...
        case 'تذكير_الصلاة':
            return await coreLoader.prayerReminderCommand.execute(interaction);
        case 'المساهمين':
            return await coreLoader.contributorsCommand.execute(interaction);
        default:
