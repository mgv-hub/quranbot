...
function isPublicFeature(interaction) {
    ...
    const isContributors = customId === 'show_contributors';

    return isAzkar || isPrayer || isPrayerMenu || isMoreFeatures || isTasbih || isContributors;
}
...
async function handlePublicInteraction(interaction) {
    try {
        ...
        if (customId === 'show_contributors') {
            await coreLoader.contributorsButton.execute(interaction);
            return true;
        }
        return false;
